const { getGenerosDB, addGeneroDB, updateGeneroDB, deleteGeneroDB,
    getGeneroPorCodigoDB} = require('../usecases/generoUseCases');
...
const getGeneros = async (request, response) => {
    await getGenerosDB()
          ...
          .catch(err => response.status(400).json({
            status : 'error',
            message : 'Erro ao consultar os generos: ' + err
          }))
}
...
const addGenero = async (request, response) => {
    await addGeneroDB(request.body)
          .then(data => response.status(200).json({
                status : "success", message : "Genero criada",
                objeto : data
          }))
          .catch(err => response.status(400).json({
            status : 'error',
...
const updateGenero = async (request, response) => {
    await updateGeneroDB(request.body)
          .then(data => response.status(200).json({
                status : "success", message : "Genero alterada",
                objeto : data
          }))
          .catch(err => response.status(400).json({
            status : 'error',
...
const deleteGenero = async (request, response) => {
    await deleteGeneroDB(request.params.codigo)
          .then(data => response.status(200).json({
...
const getGeneroPorCodigo = async (request, response) => {
    await getGeneroPorCodigoDB(request.params.codigo)
          .then(data => response.status(200).json(data))
          .catch(err => response.status(400).json({
            status : 'error',
            message : err
